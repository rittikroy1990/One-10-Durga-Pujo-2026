...
OPENAI_URL = (os.environ.get("OPENAI_BASE_URL") or "https://api.openai.com/v1").rstrip("/") + "/chat/completions"
...
def _fold(text: str) -> str:
    ...
    replacements = (
        ...
        ("committe", "committee"),
        ("commitee", "committee"),
        ("commite", "committee"),   # OCR often drops final E from COMMITEE
        ("comittee", "committee"),
        ("comitee", "committee"),
...
def org_name_match(candidate: str | None, expected_names: tuple[str, ...] | None = None) -> dict:
    ...
    has_committee = (
        "committee" in tokens
        or "committee" in folded
        or any(t.startswith("commit") for t in tokens)  # COMMITEE / COMMITE OCR truncations
    )
    token_gate = has_one10 and has_event and has_org and has_committee
    # High fuzzy score against a known bank payee spelling can pass even if OCR
    # clipped "COMMITEE" → "COMMITE" (committee token briefly incomplete).
    high_confidence = has_one10 and has_event and has_org and best_score >= 0.90

    ok = bool((token_gate or high_confidence) and best_score >= ORG_FUZZY_MIN)
    if not has_one10:
        reason = "screenshot payee is not One10 Events Organising Committee"
    elif not (token_gate or high_confidence):
    ...
    return {
        ...
        "normalized": folded,
        "reason": reason,
        "token_gate": bool(token_gate or high_confidence),
        "blocked": None,
    }


def extract_payment_screenshot(image_bytes: bytes, content_type: str = "image/jpeg") -> dict:
    """Call Emergent or OpenAI vision. Returns structured extract + org match. Never raises for business use."""
    if not EMERGENT_KEY and not OPENAI_KEY:
        return {
            "ok": False,
            "error": "EMERGENT_LLM_KEY / OPENAI_API_KEY not configured",
            "amount_paise": None,
            "utr": None,
            "payee_name": None,
            "org_name": None,
    ...
    def _ok_result(parsed: dict, model: str) -> dict:
        ...
        # Prefer payee_name (UPI "Sent to"); also try org_name and the better of both.
        candidates = [c for c in (payee_name, org_name) if c]
        if payee_name and org_name and payee_name != org_name:
            candidates.append(f"{payee_name} {org_name}")
        org_match = org_name_match(None)
        for cand in candidates:
            m = org_name_match(cand)
            if m.get("ok") or float(m.get("score") or 0) > float(org_match.get("score") or 0):
                org_match = m
            if m.get("ok"):
                break
        return {
    ...
    if EMERGENT_KEY:
        try:
            ...
            return _ok_result(parsed, MODEL)
        except Exception as e:
            logger.exception("Emergent payment screenshot LLM extract failed")
            errors.append(f"emergent:{e}")
    ...
    if OPENAI_KEY:
        try:
            ...
            content = data["choices"][0]["message"]["content"]
            parsed = _parse_json_loose(content if isinstance(content, str) else json.dumps(content))
            return _ok_result(parsed, f"openai:{MODEL}")
        except Exception as e:
            logger.exception("OpenAI payment screenshot LLM extract failed")
            errors.append(f"openai:{e}")

    return _fail("; ".join(errors) or "vision extract failed")
