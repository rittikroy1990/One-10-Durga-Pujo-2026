  ...
  const submitProof = async () => {
    ...
    try {
      ...
      if (r.data.status === "paid") toast.success(`Receipt ${r.data.receipt?.receipt_no || ""} issued`);
        ...
        toast.success(
          r.data.message
            || `Partial receipt ${r.data.receipt?.receipt_no || ""} — balance ${r.data.residual_amount_fmt || ""} due`
        );
      } else if (r.data.status === "needs_review") {
        toast.message(r.data.message || "Screenshot submitted for review — please do not pay again");
      } else toast.message(r.data.message || "Screenshot submitted for review");
                ...
                <div className="flex flex-col items-center rounded-xl border border-[#D4AF37]/35 bg-[#FFF8F0] p-4">
                  ...
                  {pay?.upi_intent_url ? (
                    <a href={pay.upi_intent_url} className="block" aria-label="Open UPI payment">
                      <img src={qrSrc} alt="Donation UPI QR" className="h-48 w-48 rounded-lg border border-[#5C3530]/10 bg-white object-contain p-1" data-testid="donate-qr-img" />
                    </a>
                  ) : (
                    <img src={qrSrc} alt="Donation UPI QR" className="h-48 w-48 rounded-lg border border-[#5C3530]/10 bg-white object-contain p-1" data-testid="donate-qr-img" />
                  )}
                  <p className="mt-3 text-center text-xs leading-relaxed text-[#5C3530]/60">
                    Scan this QR — it is locked to <span className="font-semibold">{formatPaise(upiSession.total_amount)}</span>. Use your UPI app’s <span className="font-semibold">Scan QR</span> camera.
                  </p>
                </div>
