...
async def ensure_indexes():
    ...
    await db.upi_submissions.create_index("intent_id")
    await db.upi_submissions.create_index("reference_normalized", unique=True, sparse=True)
    await db.upi_submissions.create_index("created_at")
    ...
    await db.footfall_counters.create_index("id", unique=True)
    await db.footfall_daily.create_index("id", unique=True)
    await db.footfall_daily.create_index("date")
    await db.footfall_ping_log.create_index("ts")
    await db.footfall_ping_log.create_index([("ip", 1), ("ts", 1)])
