...
export default function ReceiptFind() {
  ...
  const [lookupTxn, setLookupTxn] = useState("");
  ...
  const onLookup = async (e) => {
    ...
    const txn = lookupTxn.trim();
    if (!txn || txn.replace(/[^A-Za-z0-9]/g, "").length < 6) {
      toast.error("Enter the UPI / bank transaction ID (UTR) from your payment.");
      return;
    }
    setLookupBusy(true);
    setLookup(null);
  ...
  const onGenerate = async (e) => {
    ...
    try {
      ...
      if (start.data.status === "already_paid") {
        ...
        setStep("done");
        toast.success("Receipt already issued for this flat and transaction ID.");
        return;
      }
    ...
    <PublicLayout>
      <div className="mx-auto max-w-lg px-5 pb-16 pt-10 sm:pt-14">
        ...
        <p className="mt-2 text-brown-800/70">
          Enter your tower, flat and UPI / bank transaction ID. If a receipt exists, download it.
          If not, upload your payment screenshot to generate one.
        </p>
          ...
          >
            <div>
              ...
              <p className="mt-1 text-sm text-brown-800/65">
                Transaction ID is required — use the UTR / UPI reference from your payment app.
              </p>
            </div>
            <div>
            ...
            <div>
              <Label required htmlFor="lookup-txn">
                Transaction ID (UTR / UPI)
              </Label>
              <Input
                id="lookup-txn"
                data-testid="lookup-transaction-id"
                value={lookupTxn}
              ...
              <p className="mt-1 text-xs text-brown-800/45">
                Copy from your GPay / PhonePe / bank success screen.
              </p>
            </div>
            <Button
              type="submit"
