...
function actorLabel(e) {
  return e.actor_name || e.actor_login || e.actor_role || e.actor_id || "—";
}
                  ...
                  <tbody>
                    {(trail.events || []).map((e) => (
                      <TR key={`${e.seq}-${e.action}`}>
                        ...
                        <TD className="text-xs">{actorLabel(e)}</TD>
          ...
          <Table><THead><TR><TH>#</TH><TH>Time</TH><TH>Actor</TH><TH>Action</TH><TH>Entity</TH><TH>Entity ID</TH><TH>Reason</TH></TR></THead>
            <tbody>{events.map((e) => (
              <TR key={e.seq}><TD>{e.seq}</TD><TD>{formatDateIST(e.created_at)}</TD>
                <TD className="text-xs">{actorLabel(e)}</TD><TD className="font-mono text-xs">{e.action}</TD>
