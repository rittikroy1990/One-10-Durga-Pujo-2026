...
export default function ReceiptFind() {
  ...
  const find = async (e) => {
    ...
    try {
      ...
      toast.error(status === 429 ? "Too many attempts. Please wait a minute." : (err?.response?.data?.detail || "No receipt found with those details."));
    } finally {
      setFindBusy(false);
    }
  };
  ...
  const lookupPending = async (e) => {
    ...
    try {
      ...
      toast.error(status === 429 ? "Too many attempts. Please wait a minute." : (detail || "No pending payment found for this flat."));
    } finally {
      setLookupBusy(false);
    }
  };
  ...
  const submitProof = async (e) => {
    ...
    if (!pending?.intent_id || !pending?.status_token) {
      toast.error("Look up your flat first.");
      return;
    }
    ...
    try {
      ...
      if (r.data.status === "paid" && r.data.receipt) {
        ...
        setGenResult({
          ...
          amount: receipt.total_amount != null ? `₹${(Number(receipt.total_amount) / 100).toLocaleString("en-IN")}` : pending.amount,
          pdf_url: receipt.verify_token ? `/api/receipt/pdf/${receipt.verify_token}` : null,
      ...
      toast.error(err?.response?.data?.detail || "Could not generate receipt. Check UTR and screenshot.");
    } finally {
      setSubmitBusy(false);
    }
  };
      ...
      <div className="mx-auto max-w-lg px-5 pb-16 pt-28">
        ...
        {mode === "generate" ? (
          <div className="mt-8 space-y-5">
            <form onSubmit={lookupPending} className="space-y-4 rounded-2xl border border-sun-400/30 bg-ivory-200 p-6 text-brown-900">
              <div>
              ...
              <Button type="submit" variant="primary" className="w-full" data-testid="gen-lookup-btn" disabled={lookupBusy}>
                {lookupBusy ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Search className="h-4 w-4" /> Continue</>}
              </Button>
              <p className="text-center text-xs text-brown-800/50">
                Not subscribed yet?{" "}
                <Link to="/subscribe" className="font-semibold text-vermilion-600 underline underline-offset-2">Subscribe &amp; Pay</Link>
              </p>
            </form>

            {pending && (
              <form onSubmit={submitProof} className="space-y-4 rounded-2xl border border-sun-400/30 bg-white p-6 text-brown-900" data-testid="gen-proof-form">
                <div className="rounded-xl bg-ivory-100 px-4 py-3 text-sm">
                  <div className="font-semibold text-brown-900">
                    {pending.household?.tower_name} · Flat {pending.household?.flat_number}
                  </div>
                  <div className="mt-1 text-brown-800/70">
                    Amount due: <span className="font-semibold text-vermilion-600">{pending.amount}</span>
                  </div>
                </div>
                <ol className="space-y-1.5 text-sm text-brown-800/75">
                  <li><span className="font-semibold text-brown-900">1.</span> Pay using the Subscribe UPI QR if you have not already</li>
                  <li><span className="font-semibold text-brown-900">2.</span> Enter the UTR / UPI reference from your app</li>
                  <li><span className="font-semibold text-brown-900">3.</span> Upload the payment success screenshot</li>
                </ol>
                <div>
          ...
          <div className="mt-8">
            <form onSubmit={find} className="space-y-4 rounded-2xl border border-sun-400/30 bg-ivory-200 p-6 text-brown-900">
              ...
              <div>
                ...
                <Input id="mob" data-testid="find-mobile" value={findMobile} onChange={(e) => setFindMobile(e.target.value.replace(/\D/g, "").slice(0, 10))} placeholder="10-digit mobile" inputMode="numeric" />
              </div>
            ...
            {findResult && (
              <div className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50 p-6 text-brown-900" data-testid="find-result">
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  ...
                  <Link to={`/receipt/verify/${findResult.verify_token}`}><Button variant="subtle">Verify online</Button></Link>
                </div>
              </div>
            )}
          </div>
