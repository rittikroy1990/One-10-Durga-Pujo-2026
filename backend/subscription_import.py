"""Excel template download + upload import for subscription/donation bank transfers.
...
def build_template_xlsx(*, base_rupees: float = 3500.0) -> bytes:
    ...
    lines = [
        "",
        "1. Fill the Payments sheet (one successful bank transfer per row).",
        ...
        "5. Bank Transaction ID / UTR is required and must be unique — used to avoid duplicate receipts.",
        ...
        "9. Each successful row upserts the household and automatically issues a receipt.",
        "10. Re-uploading the same Bank Transaction ID skips that row (idempotent).",
    ]
...
def parse_import_rows(content: bytes, filename: str) -> list[dict]:
    ...
    if not rows:
        raise ValueError(
            "Could not find data rows. Required columns: Name, Tower, Flat, Amount (₹), Bank Transaction ID."
        )
    return rows
