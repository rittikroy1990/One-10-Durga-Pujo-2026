...
async def issue_for_row(row, cycle_id, settings, summary, *, dry_run: bool = False, actor=None, request=None):
    ...
    if not txn:
        ...
        return
    method = "upi_qr"
    if txn.lower() in {"cash", "cash payment", "cash-payment"}:
        method = "cash"
        tower_part = str(row.get("tower") or "").strip() or "X"
        flat_part = str(row.get("flat") or "").strip().upper().replace(" ", "") or "X"
        amt_part = str(int(round(float(row.get("amount_rupees") or 0))))
        txn = f"CASH-{tower_part}-{flat_part}-{amt_part}"
        row = {**row, "txn": txn}
    if not row.get("name"):
    ...
    pay = {
        "id": new_id("pay"),
        "provider": ("cash" if method == "cash" else "upi_qr"),
    ...
    receipt = await _issue_receipt(
        intent,
        method=method,
        masked_ref=("CASH" if method == "cash" else "UTR-" + txn[-4:]),
        provider_payment_id=txn,
        debit_account="1001",
        request=request,
        bank_verified=True,
        issuance_source="collection",
    )
    if not receipt:
        summary["errors"].append(
            {
